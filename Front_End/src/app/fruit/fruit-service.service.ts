...
export class FruitServiceService {
  ...
  private updatedfruitsdisplay = new Subject<
    { _id: string; id: string; name: string, __v: string }[]
  >();
  ...
  getfruit_service() {
    this.http
      .get<{ message: string; fruits: any }>('https://localhost:3000/api/fruits')
      .subscribe((thefruit) => {
  ...
  deletefruit_service(fruitid: string) {
    this.http
      .delete('https://localhost:3000/api/fruits/' + fruitid)
      .subscribe(() => {
